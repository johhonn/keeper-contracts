...
import './_ConditionStoreLibrary.sol';
...
contract ConditionStoreManager is Ownable, Common {
    ...
    using _ConditionStoreLibrary for _ConditionStoreLibrary.ConditionList;
    using EpochLibrary for EpochLibrary.EpochList;

    enum RoleType { Create, Update }
    address private createRole;
    _ConditionStoreLibrary.ConditionList internal conditionList;
    ...
    event ConditionUpdated(
        bytes32 indexed _id,
        address indexed _typeRef,
        _ConditionStoreLibrary.ConditionState indexed _state,
        address _who
    );
    ...
    /**
     * @dev createValueCondition only called by create role address 
     *      the condition should use a valid condition contract 
     *      address, valid time lock and timeout. Moreover, it 
     *      enforce the condition state transition from 
     *      Uninitialized to Unfulfilled.
        ...
        returns (uint size)
    {
        ...
        uint listSize = conditionList.createwithValue(_id, _typeRef,_value);
    ...
    function updateConditionState(
        bytes32 _id,
        _ConditionStoreLibrary.ConditionState _newState
    )
        external
        onlyUpdateRole(_id)
        returns (_ConditionStoreLibrary.ConditionState)
    {
        ...
        _ConditionStoreLibrary.ConditionState updateState = _newState;

        // auto abort after time out
        if (isConditionTimedOut(_id)) {
            updateState = _ConditionStoreLibrary.ConditionState.Aborted;
        }
    ...
    function getCondition(bytes32 _id)
        ...
        returns (
            address typeRef,
            _ConditionStoreLibrary.ConditionState state,
            uint timeLock,
            uint timeOut,
            uint blockNumber,
        ...
        )
    {
        ...
        EscrowValue  = conditionList.conditions[_id].EscrowValue;
    }

...
contract ServiceExecutionAgreement {
    ...
    * @param templateId , unique identifier for template generated by template owner using squid[py,js, or java]
    ...
    * @param dependenciesBits , describes the dependency model which embeds or encode the relationship between template conditions and timeout flags
    * @param fulfillmentIndices , fulfillment indices include which conditions are in charge to fulfill the agreement (mainly the reward functions)
    * @param fulfillmentOperator , the relation between conditions in the fulfillment indices (0 --> AND, 1--> OR, N--> M-of-N)
    ...
    * @param agreementId , service execution agreement ID
    ...
    * @param valueHashes , the hash of all values associated with conditions in the DID document
    * @param timeoutValues , time values if exist (zero is the default value)
    * @param agreementId , (unique identifier) service agreement instance ID
    ...
    * @param conditionKeys , array of condition keys (hash(contract_address || function selector)
    * @param valueHashes , array of hash of input values for the conditions, these values should be consistent with the
    *        passed values during the agreement initialization in order to get the same agreement hash
    * @param timeoutValues , timeout values for each condition if exists, zero is the default value
    ...
    */
    ...
    /**
    * @notice fulfillCondition called by condition fulfillment handler - authorized contract and function (conditionContract.functionSlector)
    * @dev reconstruct the condition key using the template Id, the calling address and the function fingerprint
    * checks that there is no pending fulfilment for child conditions, then lock all the children conditions to avoid replay attack
    * @param agreementId , SEA agreement ID

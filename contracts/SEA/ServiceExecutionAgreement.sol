...
contract ServiceExecutionAgreement {
    ...
    * @param fulfillmentIndices , fulfillment indices include which conditions are in charge to fulfill the
    * agreement (mainly the reward functions)
    * @param fulfillmentOperator , the relation between conditions in the fulfillment indices (0 --> AND,
    * 1--> OR, N--> M-of-N)
    ...
    * @param conditionKey , condition key is hash(contractAddress || functionSelector)
    ...
    * @param templateId , SEA template ID
    * @param contractAddress , condition handler contract address

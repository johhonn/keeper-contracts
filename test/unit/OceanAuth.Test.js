...
contract('OceanAuth', (accounts) => {
    ...
    async function prepareAccessRequestWithToken() {
        const requestId = await createAccessRequest(auth, assetId, accounts[1], 100, accounts[0])
        await auth.commitAccessRequest(requestId, true, 10000000000, '', '', '', '', { from: accounts[1] })
        await market.sendPayment(requestId, accounts[1], 0, 300, { from: accounts[0] })
        await auth.deliverAccessToken(requestId, '0x10', { from: accounts[1] })
        return requestId
    }
    ...
    beforeEach(async () => {
        ...
        auth = await OceanAuth.new(market.address, { from: accounts[0] })
    ...
    describe('initiateAccessRequest', () => {
        it('Should create access request', async () => {
            // act
            const result = await auth.initiateAccessRequest(assetId, accounts[1], 'pk', 100, { from: accounts[0] })
            ...
            const status = await auth.statusOfAccessRequest(requestId)
            assert.strictEqual(status.toNumber(), 0)
        })

        it('Should create access request for own resource', async () => {
            // act
            const result = await auth.initiateAccessRequest(assetId, accounts[0], 'pk', 100, { from: accounts[0] })
    ...
    describe('commitAccessRequest', () => {
        it('Should allow commit request by provider only', async () => {
            ...
            // act-assert
            try {
                await auth.commitAccessRequest(requestId, false, 1, '', '', '', '', { from: accounts[0] })
            } catch (e) {
        ...
        it('Should allow commit request with status Requested only', async () => {
            // arrange
            const requestId = await createAccessRequest(auth, assetId, accounts[1], 100, accounts[0])
            await auth.commitAccessRequest(requestId, false, 100, '', '', '', '', { from: accounts[1] })

            // act-assert
            try {
                await auth.commitAccessRequest(requestId, false, 1, '', '', '', '', { from: accounts[1] })
            } catch (e) {
        ...
        it('Should revoke access request if asset is not available', async () => {
            ...
            // act
            const result = await auth.commitAccessRequest(requestId, false, 100, '', '', '', '', { from: accounts[1] })
        ...
        it('Should commit access request if asset is available', async () => {
            ...
            // act
            const result = await auth.commitAccessRequest(requestId, true, 10000000000, '', '', '', '', { from: accounts[1] })
    ...
    describe('cancelAccessRequest', () => {
        it('Should allow cancel request by consumer only', async () => {
            ...
            // act-assert
            try {
                await auth.cancelAccessRequest(requestId, { from: accounts[1] })
            } catch (e) {
        ...
        it('Should allow cancel request with status Committed only', async () => {
            ...
            // act-assert
            try {
                await auth.cancelAccessRequest(requestId, { from: accounts[0] })
            } catch (e) {
        ...
        it('Should not allow cancel expired request', async () => {
            // arrange
            const requestId = await createAccessRequest(auth, assetId, accounts[1], 10000000000, accounts[0])
            await auth.commitAccessRequest(requestId, true, 10000000000, '', '', '', '', { from: accounts[1] })
        ...
        it('Should cancel access request', async () => {
            // arrange
            const requestId = await createAccessRequest(auth, assetId, accounts[1], 0, accounts[0])
            await auth.commitAccessRequest(requestId, true, 10000000000, '', '', '', '', { from: accounts[1] })
            await market.sendPayment(requestId, accounts[1], 0, 300, { from: accounts[0] })

            // act
            const result = await auth.cancelAccessRequest(requestId, { from: accounts[0] })
    ...
    describe('deliverAccessToken', () => {
        it('Should allow deliver request by provider only', async () => {
            ...
            // act-assert
            try {
                await auth.deliverAccessToken(requestId, '0x1a2e', { from: accounts[0] })
            } catch (e) {
                assert.strictEqual(e.reason, 'Sender is not Provider.')
                return
            }
        ...
        it('Should allow deliver request with status Committed only', async () => {
            ...
            // act-assert
            try {
                await auth.deliverAccessToken(requestId, '0x1a2e', { from: accounts[1] })
            } catch (e) {
                assert.strictEqual(e.reason, 'Status not Committed.')
                return
            }
        ...
        it('Should allow deliver request', async () => {
            ...
            // act
            const result = await auth.deliverAccessToken(requestId, '0x1a2e', { from: accounts[1] })
    ...
    describe('verifyAccessTokenDelivery', () => {
        it('Should allow verify access token by provider only', async () => {
            ...
            // act-assert
            try {
                await auth.verifyAccessTokenDelivery(requestId, utils.dummyAddress, utils.emptyBytes32, 0, utils.emptyBytes32, utils.emptyBytes32, { from: accounts[0] })
            } catch (e) {
                assert.strictEqual(e.reason, 'Sender is not Provider.')
                return
            }
        ...
        it('Should allow verify access token with status Delivered only', async () => {
            ...
            // act-assert
            try {
                await auth.verifyAccessTokenDelivery(requestId, utils.dummyAddress, utils.emptyBytes32, 0, utils.emptyBytes32, utils.emptyBytes32, { from: accounts[1] })
            } catch (e) {
        ...
        it('Should revoke access request if signature is not valid', async () => {
            ...
            // act
            const result = await auth.verifyAccessTokenDelivery(requestId, accounts[0], utils.emptyBytes32, 0, utils.emptyBytes32, utils.emptyBytes32, { from: accounts[1] })
        ...
        it('Should accept access request if signature is valid', async () => {
            ...
            // act
            const result = await auth.verifyAccessTokenDelivery(requestId, accounts[0], fixedMsgSha, sig.v, sig.r, sig.s, { from: accounts[1] })
    ...
    describe('getTempPubKey', () => {
        it('Should allow to call by provider only', async () => {
            ...
            // act-assert
            try {
                await auth.getTempPubKey(requestId, { from: accounts[0] })
            } catch (e) {
                return
            }
            assert.fail('Expected revert not received')
        ...
        it('Should allow to call for requests with status Committed only', async () => {
            ...
            // act-assert
            try {
                await auth.getTempPubKey(requestId, { from: accounts[1] })
            } catch (e) {
                return
            }
            assert.fail('Expected revert not received')
        ...
        it('Should return pub key', async () => {
            ...
            // act
            const result = await auth.getTempPubKey(requestId, { from: accounts[1] })
    ...
    describe('getEncryptedAccessToken', () => {
        it('Should allow to call by consumer only', async () => {
            ...
            // act-assert
            try {
                await auth.getEncryptedAccessToken(requestId, { from: accounts[1] })
            } catch (e) {
                return
            }
            assert.fail('Expected revert not received')
        ...
        it('Should allow to call for requests with status Delivered only', async () => {
            ...
            // act-assert
            try {
                await auth.getEncryptedAccessToken(requestId, { from: accounts[0] })
            } catch (e) {
                return
            }
            assert.fail('Expected revert not received')
        ...
        it('Should return encrypted access token', async () => {
            ...
            // act
            const result = await auth.getEncryptedAccessToken(requestId, { from: accounts[0] })
